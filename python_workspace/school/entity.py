# self를 항상 넣어주어야 한다!
# 부모 class (Super Class) : attribute와 operation을 자식 Class(Sub Class)에서 사용 허용(상속).
...
class Person :
    ...
    # object 클래스의 __str__ 재정의 : 재정의 하기 전 주소값 출력
    def __str__(self):    
...
class Student(Person) :
    ...
    def __str__(self):  # 다형성 : 재정의 가능, 자식에 메소드가 없다면 부모메소드 호출, 있다면 자식메소드 우선.
        return super().__str__() + "  전공:{0} ".format(self.major) 

    # @classmethod
    # def classMethod : # 호출 => 클래스 이름.메서드 : Student.classMethod
...
#__eq__ 재정의 하기 전 객체의 주소값을 비교하기 때문에 : student != student2
#__eq__ 재정의 한 후 객체의 id값 비교함으로 : student == student2
if student == student2 :
...
#           Sub가 재정의 되어있다면 Sub가 우선순위.
student.info()
teacher.info()
employee.info()

# 객체 출력 - 최상위 object class의 __str__ 호출 => 주소값 출력
#           - Sub Class에서 재정의한 경우 Sub의 재정의된 메서드가 응답된다. => 재정의한 문자열 출력.
print(student)
# print(student2)
print(teacher)
print(employee)